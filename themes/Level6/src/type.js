...
const placeSpans = () => {
  ...
  for (var i = 0; i < textLength; i++) {
	...
	let x = guideSpans[0].getBoundingClientRect();
	if(x.top != rect.top) {
		animatedStroke.style.top = (rect.top-x.top) + 'px';
		animated.style.top = (rect.top-x.top) + 'px';
	} else {
		animatedStroke.style.top = 0 + 'px';
		animated.style.top = 0 + 'px';
	}
  }
}
